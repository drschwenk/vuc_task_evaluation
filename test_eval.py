import pytest
import os.path
from vuc_task_evaluation.score_submission import EvaluatorSelector
...
@pytest.mark.parametrize("submission, gt_path, expected", [
        (os.path.join(tqa_base_path, 'random.json'), tqa_gt_path, (0.23299050632911392, 0.24487594390507011)),
        (os.path.join(tqa_base_path, 'ndq_only.json'), tqa_gt_path, (1.0, 0)),
        (os.path.join(tqa_base_path, 'dq_only.json'), tqa_gt_path, (0.0, 1.0)),
        (os.path.join(tqa_base_path, 'perfect.json'), tqa_gt_path, (1.0, 1.0))])
def test_tqa_submissions(evaluator, submission, expected):
    accuracies = evaluator.evaluate_submission()
    print(list(accuracies))
    assert accuracies[0][1] == expected[0]
    assert accuracies[1][1] == expected[1]


@pytest.mark.parametrize("submission, gt_path, expected", [
    # (os.path.join(tqa_base_path, 'missing_vals.json'), tqa_gt_path, AssertionError), changed the task, not needed now
